...
module.exports = class CardStats extends expose.Component {
	...
	getHeaderText(statName) {
		if (statName === this.state.sortField) {
			const ret = STAT_ALIASES[statName] + (this.state.sortDirection === 'DSC' ? DSC_CHAR : ASC_CHAR);
			return ret;
		} else {
	...
	getCellClassName(statName) {
		...
		if (statName === 'name') {
			...
			className = className + ` ${STATS_NAMES.slice(0, 2).includes(statName) ? 'percentage' : 'number'}-stat-cell`;
		}
		return className;
	}
	...
	renderStatsHeader() {
			...
			return (
				<Cell key={statName}>
					<div
						...
						<span style={{ userSelect: 'none' }}>{this.getHeaderText(statName)}</span>
					</div>
				</Cell>
			);
		});
		...
		return (
			<Row key="header" style={{ cursor: 'default', color: 'white', display: 'flex', fontWeight: 'bold' }}>
				{elems}
			</Row>
		);
	}
	...
	renderPlayerRow(playerStats) {
		const elems = ['name'].concat(STATS_NAMES).map((statName, i) => {
			return (
				<Cell key={statName}>
					<div
						className={this.getCellClassName(statName)}
						onClick={!i ? this.handlePlayerClick.bind(this, playerStats.id) : null}>
		...
		return (
			<Row key={playerStats.name} style={{ cursor: 'default', color: 'white', display: 'flex' }}>
				{elems}
			</Row>
		);
	}
	...
	buildStatsObject(teamId, playerId) {
		...
		const plateAppearances = state.getPlateAppearancesForPlayerOnTeam(playerId, teamId);
