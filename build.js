...
const rules = {
  ...
  "build-vendor": function(cb) {
    ...
    const cmd = `${NODE_PATH} browserify ${_vendor_exports().join(
      " "
    )} | babel-minify --mangle true > ${vendor_bundle}`;
    _execute(cmd, cb);
  },
  "build-prod": function(cb) {
    rules["build-vendor"](function(error) {
      ...
      const cmd = `${NODE_PATH} browserify ${_vendor_imports().join(
        " "
      )} ${entry} | babel-minify --mangle true > ${bundle}`;
      _execute(cmd, cb);
    });
  },
  build: function(cb) {
    const entry = process.env.npm_package_config_bundles_main_entry;
    const bundle = process.env.npm_package_config_bundles_main_out;
    const cmd = `${NODE_PATH} browserify ${_dev_flags()} ${_vendor_imports().join(
      " "
    )} ${entry} | babel-minify --mangle true > ${bundle}`;
    initConfigFiles(err => {
      if (err) {
  ...
  watch: function(cb) {
    const entry = process.env.npm_package_config_bundles_main_entry;
    const bundle = process.env.npm_package_config_bundles_main_out;
    const cmd = `${NODE_PATH} watchify ${_dev_flags()} ${_vendor_imports().join(
      " "
    )} ${entry} -o ${bundle}`;
    _execute(cmd, cb);
  },
...
function _vendor_imports() {
  return [
    " -t [ babelify --sourceMapsAbsolute --presets [ @babel/preset-react ] ] "
  ].concat(
