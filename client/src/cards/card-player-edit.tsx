...
interface CardPlayerEditProps {
  player: Player;
  isNew: boolean;
}

interface CardPlayerEditState {
  playerGender: string;
  playerName: string;
  playerSongLink: string;
  playerSongStart: string | number;
  youTubeKey: number;
}
...
> {
  isNew: boolean;
  isPristine: boolean;
  queryYouTube: (query: string) => Promise<YouTubeOption[]>;
  formatOptionLabel: (
  ...
  handlePlayerNameChange: (value: string) => void;
  handleGenderChange: (ev: React.ChangeEvent<HTMLInputElement>) => void;
  handleYouTubeSelection: (selectedOption: YouTubeOption) => void;
  isValidYouTubeVideoLink: (url: string) => boolean;
  handleSongStartChange: () => void;
  handleSongHelpClick: () => void;

  constructor(props: CardPlayerEditProps) {
    super(props);
