...
const CardOptimizationEdit: React.FC<CardOptimizationEditProps> = (props) => {
  ...
  const buildOptimization = function (): Optimization {
    const optimization = JSON.parse(JSON.stringify(props.optimization));
    optimization.name = optName;
    return optimization;
  };

  const homeOrBack = function (type: string): boolean | void {
    if (!isPristine) {
      dialog.show_confirm(
        props.isNew
  ...
  const handleCancelClick = function (): void {
    homeOrBack('back');
  };
  ...
  const handleDuplicateClick = function (): void {
    dialog.show_confirm(
  ...
  const handleOptimizationNameChange = (value: string): void => {
    setIsPristine(false);
    setOptName(value);
  };
