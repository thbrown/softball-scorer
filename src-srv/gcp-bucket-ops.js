...
const gcpOps = class GcpOps {
  ...
  static async readBlob(accountId, targetBucket, blobName, schemaToValidate) {
    try {
  ...
  static async deleteBlob(accountId, targetBucket, blobName) {
    try {
  ...
  static async exists(accountId, targetBucket, blobName) {
    try {
      // Now delete the actual file
      const file = targetBucket.file(blobName);
