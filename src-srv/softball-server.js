...
module.exports = class SoftballServer {
	...
	start() {
		...
		app.use(
			...
			express.static(path.join(__dirname + '/../src/workers/simulation-worker.js').normalize()),
		);
		app.use(
			bodyParser.json({
				limit: '3mb',
				...
				if (config && config.recapcha && config.recapcha.secretkey) {
					...
					try {
						const recapchaResponse = await got.post(
							`https://www.google.com/recaptcha/api/siteverify?secret=${config.recapcha.secretkey}&response=${
								req.body.reCAPCHA
							}`,
						);
