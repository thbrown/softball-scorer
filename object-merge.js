 ...
 *
 * skipOperationOnNonExistant - if true will skip patch operations where the data to be patched does not exist in the patch object. If false this and error will be thrown instead.
 * skipDeletes - don't do delete operations if true (usefully for doing merges)
 */
let patch = function(toPatch, patchObj, skipOperationOnNonExistant, skipDeletes) {
	Object.keys(patchObj).forEach(function(key) {
		if(isLeaf(patchObj[key])) {
			...
			patch(toPatchSubtree,patchObj[key],skipOperationOnNonExistant,skipDeletes);
		}
	});
