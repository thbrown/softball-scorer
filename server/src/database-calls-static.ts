...
export default class DatabaseCalls {
  ...
  async signup(email, passwordHash, passwordTokenHash) {
    ...
    this.idCounter++;
    logger.log('', 'Account added', JSON.stringify(this.ACCOUNTS, null, 2));
  ...
  async getNumberOfOptimizationsInProgress(accountId) {
    ...
    // I don't know where this was supposed to come from
    // const state = this.STATES[accountId];
    // let count = 0;
  ...
  async disconnect() {
    logger.log('', 'disconnecting from static db (NO-OP)');
  }
}
